<template>
 <div>
    ...
    <BasicTable @register="registerTable" :rowSelection="rowSelection" :searchInfo="searchInfo">
      ...
      <EduClassSubModal @register="registerModal" @success="handleSuccess"/>
   </div>
</template>
...
<script lang="ts" setup>
  ...
  import EduClassSubModal from './components/EduClassSubModal.vue'
  import {eduClassSubColumns} from './EduClass.data';
  import {eduClassSubList, eduClassSubDelete, eduClassSubDeleteBatch, eduClassSubExportXlsUrl, eduClassSubImportUrl } from './EduClass.api';
  ...
  import {downloadFile} from '/@/utils/common/renderUtils';
    ...
    const {prefixCls, tableContext, onImportXls, onExportXls} = useListPage({
        ...
        importConfig: {
            url: ()=>{
                return eduClassSubImportUrl + '/' + unref(mainId)
            }
        }
    });
    ...
    async function handleDelete(record) {
        await eduClassSubDelete({id: record.id}, handleSuccess);
    }
    ...
    async function batchHandleDelete() {
        await eduClassSubDeleteBatch({ids: selectedRowKeys.value}, handleSuccess)
    }
