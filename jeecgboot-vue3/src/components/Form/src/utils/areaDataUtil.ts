...
const regionData: { value: string; label: string; children?: any[] }[] = [];
let provinceAndCityData: { value: string; label: string; children?: any[] }[] = [];
...
for (let i = 0, len = regionData.length; i < len; i++) {
  ...
  const provinceChildren: { value: string; label: string; children?: any[] }[] = [];
...
for (let i = 0, len = regionData.length; i < len; i++) {
  ...
  if (province) {
    for (let j = 0, len = province.length; j < len; j++) {
      ...
      //const cityChildren = [];
      const cityChildren: { value: string; label: string; children?: any[] }[] = [];
...
const provinceOptions: { value: string; label: string; children?: any[] }[] = [];
for (const prop in provinceObject) {
...
function getDataByCode(code) {
  //let data = [];
  const data: { value: string; label: string; children?: any[] }[] = [];
 ...
 * @type {Array}
 */
type PcaItem = { id: string; text: any; pid: string; index: number };
const pca: PcaItem[] = [];
